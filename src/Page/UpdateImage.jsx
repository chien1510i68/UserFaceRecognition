...
function UpdateImage(props) {
  ...
  const [loading, setLoading] = useState(true);
  const [isTest, setIsTest] = useState(false);
  const [isGetImages, setIsGetImage] = useState(false);
  ...
  const handleGetImages = async () => {
    ...
    if (res?.data?.success) {
      setLoading(false);
      const listImage = res?.data?.data?.map((item) => {
        return {
          name: item?.file_name?.split("/")[1],
          image: item?.encoded_image,
        };
      ...
      setImages(listImage);
      // console.log("list image ",res?.data?.data);
      setIsShowImage(true);
    } else {
      setLoading(false);
      setIsGetImage(true);
  ...
  return (
    <div>
      ...
      {isGetImages && (
        <div>
          <div>
            <GetImagesToTrain handleTest={() => {setIsTest(true); setIsGetImage(false)}} />
          </div>
        </div>
      )}
